...
export const Order = () => {
  ...
  const OnClickChangeDataEntrega = () => {
    editDataEntrega === "Pendente"
      ? setEditDataEntrega(dataAtual)
      : setEditDataEntrega("Pendente");
  };

  const OnClickChangeStatus = () => {
    editStatus === "Pronto"
      ? setEditStatus("Pendente")
      : setEditStatus("Pronto");
  };
  ...
  const OnleDelete = (osId) => {
    axios.delete(`http://localhost:8000/orders/${osId}`);
      ...
      <div>
        <div className="order-container">
          <div className="order-header">
            ...
            <p>Valor: R$ {parseFloat(parseFloat(order.amount)).toFixed(2)} </p>
            {formAddVisible && (
              <FormAddItem id={osId} OnSaveEdit={OnSaveEdit} save={save} />
            )}
            {visible && (
              <table>
                <thead>
                ...
                <div className="finish-os-container">
                  ...
                  <div className="checkbox-pay-container">
                    <input
                      ...
                      name="payed"
                      id="payed"
                      onClick={() => OnPay(OnSaveEdit("payed"))}
                    />
                    <span>
            ...
            <tbody>
              {order.items &&
                order.items.map((item, key) => (
                  ...
                  >
                    <td
                      className="item-Edit td-width-60"
                      onClick={(e) =>
                        OnClickEdit(
                          setEditQuantity(item.quantity),
                          setEditDataEntrega(order.data_entrega),
                    ...
                    {visible && (
                      <td
                        className="td-width-60 btn-delete"
